...
(also installed the Wes Anderson colour palette; credit: https://github.com/karthik/wesanderson)
```
...
To get the basic attributes of each data set I can use the _str()_ function which gives the number of rows, columns, the names of columns and, the type of variables used.
...
```{r}
steam_games %>% arrange(release_date) %>% select(release_date) %>% head(n=8)
```

Interestingly, some of the dates are not even dates but rather *coming soon* or even "¯\\_(ツ)_/¯"

This means date's will have to be edited or even changed before they can be analyzed.
...
Although this data set has a lot of information, I do not think I want to use it further since I cannot think of a lot of questions I could ask or answer with the given information.
...
Interestingly again the column names do not make sense to someone not already familiar with oncology and even then there is no further information given so I would prefer to not use this for my analysis
...
For my final choice, I find _vancouver_trees_ the most interesting and I feel that it has the most potential to answer questions about people's reviews of recent games, and genres.  
...
```{r}
vancouver_trees %>% ggplot(aes(diameter))+ geom_histogram(bins = 20,na.rm = TRUE, colour = "black")+ xlim(0,60)+xlab(label = "Diameter")
```
Here we can see that most of the trees have diameters between 20 and 0. No units were provided so I am assuming they mean inches.
...
trees_avg_dia = vancouver_trees %>% group_by(species_name)%>%mutate(avg_diameter=mean(diameter, na.rm=TRUE))
trees_avg_dia %>% select(species_name, diameter, avg_diameter)  
```
...
print(as.tibble(species_count_name))
